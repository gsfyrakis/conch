...
public class EventsMDB implements MessageListener {
	...
	public void onMessage(Message rcvMessage) {
		...
		try {
			if (rcvMessage instanceof ObjectMessage) {
				...
				ccc = ContractComplianceChecker.createContractComplianceChecker(ConfigurationFilesEnum.BUYER_STORE_CONTRACT.getConfigurationFilePath());
	 ...
	 */
	private BusinessEvent receiveBusinessEventMsg(BytesMessage rcvMessage) throws JAXBException, JMSException {
	 ...
	 *
	 * @param bEvent the business event
	 * @return the event
	 */
	 ...
	 *             the jMS exception
	 */
	private String getStringFromBytesMessage(BytesMessage rcvMessage) throws JMSException {

		log.info("message received: " + rcvMessage.toString());
		BytesMessage bmsg =  rcvMessage;
		byte[] bytes;
